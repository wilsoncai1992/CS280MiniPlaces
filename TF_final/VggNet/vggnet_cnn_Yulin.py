# -*- coding: utf-8 -*-
"""
Created on Wed Mar  1 10:09:16 2017

@author: Yulin Liu

VggNet: Modified from AlexNet created by Wilson
"""
import Dataset
import os
import sys
...
class ConvNet(object):
    ...
    def __init__(self, learning_rate, max_epochs, display_step, std_dev, dataset):
        ...
        self.weights = {
            ...
            'wc1_2': tf.Variable(tf.random_normal([3, 3, 64, 64], stddev=std_dev)),
            'wc2_1': tf.Variable(tf.random_normal([3, 3, 64, 128], stddev=std_dev)),
            'wc2_2': tf.Variable(tf.random_normal([3, 3, 128, 128], stddev=std_dev)),
            ...
            'wd': tf.Variable(tf.random_normal([4*4*512, 4096])),
        ...
        self.biases = {
            ...
            'bd': tf.Variable(tf.zeros([4096])),
            'bfc': tf.Variable(tf.zeros([4096])),
...
#        return tf.nn.lrn(l_input, depth_radius = lsize, bias=2.0, alpha=0.0001, beta=0.75, name=name)
    ...
    def vgg_net_model(self, _X, _weights, _biases, _dropout):        
    ...
    def training(self):
        ...
        with tf.Session() as sess:
            # Construct model
            logits, prediction = self.vgg_net_model(self.img_pl, self.weights, self.biases, self.keep_prob)
...
#            optimizer = tf.train.MomentumOptimizer(learning_rate=self.learning_rate, momentum=0.9).minimize(loss)
    ...
    def prediction(self):
        with tf.Session() as sess:
            # Construct model
            pred, _ = self.vgg_net_model(self.img_pl, self.weights, self.biases, self.keep_prob)
