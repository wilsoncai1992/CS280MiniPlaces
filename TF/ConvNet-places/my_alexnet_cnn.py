...
IMAGE_DIR = os.getcwd() + '/images/train/b/'
...
class ConvNet(object):
    ...
    def __init__(self, learning_rate, max_epochs, display_step, std_dev, dataset):
        ...
        # self.label_pl = tf.placeholder(tf.float64, [None, n_classes])
        self.img_pl = tf.placeholder(tf.float32, [None, n_input, n_channels])
    ...
    def prediction(self):
        with tf.Session() as sess:
            ...
            ckpt = tf.train.get_checkpoint_state("/tmp/tf_logs/ConvNet/")
            if(ckpt):
            ...
            for dirName in os.listdir(IMAGE_DIR):
                ...
                for img in os.listdir(path):
                    ...
                    if(os.path.isfile(img_path) and (img_path.endswith('jpeg') or
                        ...
                        # WILSON: top 5 manual.......
                        predict = sess.run(prod, feed_dict={self.img_pl: [img_eval], self.keep_prob: 1.})
